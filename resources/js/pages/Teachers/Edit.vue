...
const form = useForm({
    user_id: props.teacher.user_id ? String(props.teacher.user_id) : '',
    name: props.teacher.name,
    phone: props.teacher.phone ?? '',
    status: props.teacher.status,
    note: props.teacher.note ?? '',
});
...
<template>
    ...
    <div class="page-shell mx-auto w-full max-w-3xl">
        <h1 class="text-xl font-semibold">編輯老師</h1>
        <form @submit.prevent="form.put(`/teachers/${props.teacher.id}`)" class="space-y-4 rounded-xl border p-4">
            ...
            <div class="grid gap-2">
                ...
                <select id="user_id" v-model="form.user_id" class="h-9 rounded-md border px-3">
                    ...
                    <option v-for="u in teacherUsers" :key="u.id" :value="String(u.id)">{{ u.name }} ({{ u.email }})</option>
                </select>
