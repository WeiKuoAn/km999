...
import { formatCourseSchedules, type CourseScheduleRow } from '@/lib/weekdayDates';
...
const scheduleLabel = (course: Course) => {
    ...
    if (course.weekdays?.length) {
        return formatCourseSchedules(course.weekdays.map((d) => ({ weekday: d })));
    }
    return '—';
};

const statusLabel = (status: Course['status']) => (status === 'active' ? '啟用' : '停用');
    ...
    <div class="page-shell">
        ...
        <div class="mobile-card-list">
            ...
            >
                ...
                <MobileRecordField label="年級">{{ levelLabel(course.course_prices) }}</MobileRecordField>
                <MobileRecordField label="上課時段">{{ scheduleLabel(course) }}</MobileRecordField>
                <MobileRecordField label="收費組別">{{ pricingGroupLabel(course.pricing_group) }}</MobileRecordField>
                <template v-if="canManage" #actions>
                    <div class="mobile-card-actions">
                        <TableEditIconLink :href="`/courses/${course.id}/edit`" />
                        <TableDeleteIconButton @click="destroyCourse(course.id)" />
                    </div>
                </template>
            </MobileRecordCard>
            <p
        ...
        <div class="desktop-table-wrap">
            <table class="w-full text-sm">
                ...
                <tbody>
                    <tr v-for="course in courses.data" :key="course.id" class="border-b">
                        <td class="py-2">
                            <span
                        ...
                        <td class="py-2">{{ levelLabel(course.course_prices) }}</td>
                        <td class="py-2">{{ scheduleLabel(course) }}</td>
                        <td class="py-2">{{ pricingGroupLabel(course.pricing_group) }}</td>
